...
class APIClient(
    ...
    override suspend fun <T : Any> postJson(path: String, body: T): Pair<String, HttpURLConnection> {
        val url = try { URL(env.baseUrl + path) } catch (e: Exception) { throw NetworkError.InvalidUrl }
        val conn = (url.openConnection() as HttpURLConnection).apply {
            ...
            tokenProvider.accessToken?.let { setRequestProperty("Authorization", "Bearer $it") }
            doOutput = true
