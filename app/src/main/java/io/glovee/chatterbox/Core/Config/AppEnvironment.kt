package com.chatterboxtalk.Core.Config
...
class AppEnvironment(context: Context) {
    ...
    // Reviewer email for instant login (optional)
    ...
    init {
        ...
        // Optional reviewer email for instant login (no magic link flow)
        val reviewerEmailId = res.getIdentifier("reviewer_email", "string", pkg)
        reviewerEmail = if (reviewerEmailId != 0) {
            try {
                res.getString(reviewerEmailId).takeIf { it.isNotBlank() }
            } catch (_: Exception) {
                null
            }
        } else null
